use crate::{NodeIndex, Split, Style, TabDestination, TabIndex};
...
impl HoverData {
    ...
    pub(super) fn resolve(&self, ui: &mut Ui, style: &Style) -> TabDestination {
        if self.is_on_title_bar() {
            self.resolve_traditional(ui, style)
        } else {
            self.resolve_icon_based(ui, style)
        }
    }

    fn resolve_icon_based(&self, ui: &mut Ui, style: &Style) -> TabDestination {
